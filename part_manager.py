...
def submit(): 
   ...
   c = conn.cursor()
   # Insert Into Table
   c.execute("INSERT INTO addresses VALUES (:name, :apellido_entry, :dni_entry, :matricula_entry)", 
         {
...
matricula_entry = Entry(app, textvariable=dni_text)
matricula_entry.grid(row=1, column=4)

# *Botones*
# Quit 
exit_button = Button(app, text="SalirA", fg="red", command= app.destroy)
exit_button.grid(row=15, column=0, columnspan=2, padx=20, ipadx=137)

# Añadir a la base datos 
submit_btn = Button(app, text="Añadir a la base de datos", command= submit)
submit_btn.grid(row = 6, column=0, columnspan=2, pady=20, padx= 10, ipadx= 137)
